...
private fun shouldOpenKeyboard(
    ...
    frame: TerminalFrame?,
    event: MotionEvent
): Boolean = shouldOpenKeyboardForTap(
    autoShowKeyboardOnTap = config.autoShowKeyboardOnTap,
    unconditionalKeyboardOnTap = config.unconditionalKeyboardOnTap,
    mouseTrackingActive = frame?.mouseTrackingActive == true,
    isMouseSource = event.isFromSource(InputDevice.SOURCE_MOUSE)
)

/** Pure tap-to-keyboard policy; unit-tested without framework events. */
internal fun shouldOpenKeyboardForTap(
    autoShowKeyboardOnTap: Boolean,
    unconditionalKeyboardOnTap: Boolean,
    mouseTrackingActive: Boolean,
    isMouseSource: Boolean
): Boolean {
    if (!autoShowKeyboardOnTap) return false
    if (isMouseSource) return false
    return unconditionalKeyboardOnTap || !mouseTrackingActive
}
